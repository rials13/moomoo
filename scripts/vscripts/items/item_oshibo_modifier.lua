item_oshibo_modifier = class({})

--------------------------------------------------------------------------------
-- Classifications
function item_oshibo_modifier:IsHidden()
	return true
end

function item_oshibo_modifier:IsDebuff()
	return false
end

function item_oshibo_modifier:IsPurgable()
	return false
end
...
function item_oshibo_modifier:OnCreated( kv )
...
function item_oshibo_modifier:OnRefresh( kv )
	-- references
	
end

function item_oshibo_modifier:OnDestroy( kv )

end

--------------------------------------------------------------------------------
-- Modifier Effects
function item_oshibo_modifier:DeclareFunctions()
	local funcs = {
...
function item_oshibo_modifier:GetModifierPreAttack_BonusDamage( params )
	return self.damage
end

function item_oshibo_modifier:GetModifierBonusStats_Strength( params )
	return self.strength
end
--------------------------------------------------------------------------------

function item_oshibo_modifier:OnDeath( params )
...
function item_oshibo_modifier:KillLogic( params )
	-- filter
...
function item_oshibo_modifier:AddStack( stat_gain )
	-- update strength
	self:GetParent():SetBaseStrength( self:GetParent():GetBaseStrength() + stat_gain )
	print("gained some strength")

	-- update modifier stack count
	-- from pudge flesh heap
	local hBuff = self:GetParent():FindModifierByName( "item_oshibo_modifier_stack" )
	if hBuff == nil then
		-- Add Modifier
		local mod = self:GetParent():AddNewModifier(
			self:GetParent(),
			self:GetAbility(),
			"item_oshibo_modifier_stack", {}
		)
		mod.modifier = self
	else
...
--------------------------------------------------------------------------------
function item_oshibo_modifier:PlayEffects()
...
--------------------------------------------------------------------------------
-- Helper
function item_oshibo_modifier:RollChance( chance )
